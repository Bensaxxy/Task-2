...
import { CartContext } from "../Home/CartContext"; // Import the CartContext
...
const Cart = () => {
  ...
  const totalAmount = cartItems.reduce(
    (total, item) =>
      total + parseInt(item.amount.replace("NGN ", "").replace(",", "")),
    0
  );
        ...
        <div className="border border-primary/10 rounded-md grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-between">
          ...
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item) => (
              <div
                key={item.id}
                className="hidden lg:flex lg:col-span-2 justify-between py-8 px-4 items-center flex-col lg:flex-row gap-6"
              >
                <div className="bg-primary/30 p-4 rounded-md">
                  <img src={item.image} className="w-[160px]" />
                </div>
                ...
                <div>
                  ...
                  <button
                    className="flex items-center justify-center rounded-md bg-red-200 py-1 px-4 border border-red-500 text-red-500 gap-2"
          ...
          <div className="py-8 px-6 border">
            ...
            <div className="flex justify-between">
              ...
              <p className="text-primary text-md">
                (20%) -NGN {(totalAmount * 0.2).toLocaleString()}
              </p>
            </div>
            <div className="flex justify-between">
              <p className="text-primary text-md">
              ...
              <p className="text-primary text-md">
                +NGN {(totalAmount * 0.07).toLocaleString()}
              </p>
            </div>
